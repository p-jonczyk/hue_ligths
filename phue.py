...
class Bridge(object):
    ...
    def __init__(self, ip=None, username=None, config_file_path=None):
        ...
        if config_file_path is not None:
            ...
            self.config_file_path = os.path.join(
                os.getenv(USER_HOME), 'Documents', '.python_hue')
        else:
    ...
    def request(self, mode='GET', address=None, data=None):
        ...
        try:
            ...
            error = "{} Request to {}{} timed out.".format(
                mode, self.ip, address)
    ...
    def create_sensor(self, name, modelid, swversion, sensor_type, uniqueid, manufacturername, state={}, config={}, recycle=False):
        ...
        result = self.request(
            'POST', '/api/' + self.username + '/sensors/', data)
    ...
    def get_sensor(self, sensor_id=None, parameter=None):
        ...
        if isinstance(data, list):
            logger.debug("Unable to read sensor with ID {0}: {1}".format(
                sensor_id, repr(data)))
            return None
    ...
    def set_sensor_content(self, sensor_id, parameter, value=None, structure="state"):
        ...
        if (structure != "state" and structure != "config"):
            logger.debug(
                "set_sensor_current expects structure 'state' or 'config'.")
            return False
    ...
    def delete_sensor(self, sensor_id):
        try:
            ...
            logger.debug(
                "Unable to delete nonexistent sensor with ID {0}".format(sensor_id))
        ...
        for group in group_id_array:
            ...
            if parameter == 'name' or parameter == 'lights':
                result.append(self.request(
                    'PUT', '/api/' + self.username + '/groups/' + str(converted_group), data))
            else:
                result.append(self.request('PUT', '/api/' + self.username +
                              '/groups/' + str(converted_group) + '/action', data))
    ...
    def run_scene(self, group_name, scene_name, transition_time=4):
        ...
        if len(groups) != 1:
            logger.warn(
                "run_scene: More than 1 group found by name {}".format(group_name))
            return False
        group = groups[0]
        if len(scenes) == 0:
        ...
        if len(scenes) == 1:
            self.activate_scene(
                group.group_id, scenes[0].scene_id, transition_time)
            return True
        ...
        for scene in scenes:
            if group_lights == scene.lights:
                self.activate_scene(
                    group.group_id, scene.scene_id, transition_time)
                return True
